...
class DirOps:
    """Dirops class retrieves basic information, performs operations on path
    ...
    ...
    def __init__(self, path):
        ...
        self.path = path
        self.subdirs = sorted([name for name in os.listdir(path) if os.path.isdir(os.path.join(path, name))])
        self.files = sorted([name for name in os.listdir(path) if not os.path.isdir(os.path.join(path, name))])

        self.nsubdirs = len(self.subdirs)
        self.nfiles = len(self.files)
        self.dirsize = subprocess.check_output(['du', '-sh', self.path]).split()[0].decode('utf-8')
    ...
    def dirinfo(self):
        ...
        print(f"Subdirs: {self.nsubdirs} | Files: {self.nfiles} | Recursive files: {self.titems} ")
    ...
    def lsmedia(self):
        ...
        img_fns, vid_fns, aud_fns = fns(self.path, "image"), fns(self.path, "video"), fns(self.path, "audio")
    ...
    def del_files(self, reflist, match=True):
        ...
        match : bool
            flag whether to delete files that match
        """
