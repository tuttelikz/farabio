...
class UnetTrainer(ConvnetTrainer):
    def init_attr(self):
        ...
        self._semantic = config.semantic
        self._model_save_dir = os.path.join(config.model_save_dir)
        self._num_gpu = config.num_gpu
        self._device = torch.device(config.device)
        self._cuda = config.cuda
        self._num_workers = config.nw
        self._data_parallel = config.data_parallel
        self._output_img_dir = config.output_img_dir
        self._output_mask_dir = config.output_mask_dir
        self._output_overlay_dir = config.output_overlay_dir
        self._model_load_dir = config.model_load_dir
        self._use_visdom = config.use_visdom
    ...
    def build_model(self):
        ...
        #print(summary(self.model, [(3, 512, 512)]))
    ...
    def training_step(self):
        ...
        #imgs_device_id = self.imgs.device.index
        #model_device_id = self.model.dummy_param.get_device()

        #print("img: ", imgs_device_id, " model: ", model_device_id)
    ...
    def save_model(self):
        ...
        """
        if self._semantic:
            model_name = os.path.join(self._model_save_dir, "semunet.pt")
        elif not self._semantic:
            model_name = os.path.join(self._model_save_dir, "unet.pt")
...
class LoadDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = self.get_mask(mask_folder, 512, 512).astype('float32')
...
if __name__ == "__main__":
    ...
    config, unparsed = get_config('unet')
