...
#from farabio.kernel import config as config
...
class FasterRCNN(nn.Module):
    ...
    def predict(self, imgs, sizes=None, visualize=False):
        ...
        if visualize:
            ...
            for img in imgs:
                size = img.shape[1:]
                img = preprocess(helpers.tonumpy(img))
        ...
        for img, size in zip(prepared_imgs, sizes):
            img = helpers.totensor(img[None]).float()
            scale = img.shape[3] / size[1]
            roi_cls_loc, roi_scores, rois, _ = self(img, scale=scale)
            # We are assuming that batch size is 1.
            roi_score = roi_scores.data
            roi_cls_loc = roi_cls_loc.data
            roi = helpers.totensor(rois) / scale
            ...
            cls_bbox = loc2bbox(helpers.tonumpy(roi).reshape((-1, 4)),
                                helpers.tonumpy(roi_cls_loc).reshape((-1, 4)))
            cls_bbox = helpers.totensor(cls_bbox)
            cls_bbox = cls_bbox.view(-1, self.n_class * 4)
            # clip bounding box
            cls_bbox[:, 0::2] = (cls_bbox[:, 0::2]).clamp(min=0, max=size[0])
            cls_bbox[:, 1::2] = (cls_bbox[:, 1::2]).clamp(min=0, max=size[1])

            prob = (F.softmax(helpers.totensor(roi_score), dim=1))
    ...
    def get_optimizer(self):
        ...
        lr = config.lr
        params = []
        for key, value in list(dict(self.named_parameters()).items()):
